...
def design1():                  #2 engine tiltrotor, 
    ...
    fuselage1 = [fuselage(2000,(-5.5,0,0))]
...
def design2_2():
    rotors = []
    engine1 = engine(240, (-1.5,6.5,-1), 475)        #0.24kg/kw/h
    engine2 = engine(240, (-1.5,-6.5,-1), 475)
    engine3 = engine(240, (-17,-6.5,-1), 475)
    engine4 = engine(240, (-17,6.5,-1), 475)
    rotors.append(rotor(engine1, 12,engine1.cg, 120, 0.65))
    rotors.append(rotor(engine2, 12,engine2.cg, 120, 0.65))
    rotors.append(rotor(engine3, 12,engine3.cg, 120, 0.65))
    rotors.append(rotor(engine4, 12,engine4.cg, 120, 0.65))
    wings = []
    fueltanks = []
    airfoil1 = airfoil(0.08, 2.4)
    wings.append(wing(airfoil1, (-1.5,0,-1), 12,3))
    wings.append(wing(airfoil1,(-15,0,-3), 12,3))
    fueltanks.append(fueltank(900,(-1.5,0,-1)))
    fueltanks.append(fueltank(660,(-15,0,-3)))
    return UAV(rotors, wings, container(5000, (-8.5,0,1.25),1.2), fueltanks, [fuselage(1500,(-8.5,0,-1))],110000,)
...
class rotor(object):            #engine, diameter, location, mass, merit, thrustlv; Functions: st_thrust(a), set_thrust(tl), tilt(dtetha)
    def __init__(self, engine, diameter = 1, location = (0,0,0), mass = 200, merit = 0.7, thrustlv = 1, t=0,c=0):         #DOF, Angle, Angle adjustment speed
...
class airfoil(object):          #Cd0, cl_max, cl_0; Functions: Cl_lvl(a, wing), Cl_alpha(a)
    def __init__(self, Cd0 = 0.0315, cl_max = 1.5, cl_0 = 0):
        self.Cd0 = Cd0
        self.cl_max = cl_max
        self.cl_0 = cl_0        
...
def mindrag(a, x = 0):
    V,d1,d2,d3,d4, cl1, cl2, pr, rc, d5 = [],[],[],[],[],[],[],[],[],[]
    for i in range (1,60):
        a.setV(i)
        c=a.cruise()
        d1.append(c[0])
        ...
        if a.wings[0].Cl < a.wings[0].airfoil.cl_max:
            pr.append(c[5])
            rc.append(a.climb())
            ...
            for wing in a.wings:
                l += wing.lift(a)
            pr.append(a.power(a.W-l)+c[5])
            rc.append(1)
    if x == 1:
        plt.plot(V,d1,color = "blue")
        plt.plot(V,d2,color = "red")
        plt.plot(V,d3,color = "green")
        plt.plot(V,d4,color= "cyan")
        plt.plot(V,d5,color = "purple")
